...
const commentsBase = (p: number, b: number, c: number, card: number) =>
  `${FULL_PREFIX}/projects/${p}/kanban/boards/${b}/columns/${c}/cards/${card}/comments`;
...
describe('CommentsApi', () => {
  ...
  it('listComments GETs the comments endpoint and unwraps data', async () => {
    const promise = firstValueFrom(api.listComments(7, 4, 12, 87));
    const req = httpMock.expectOne(commentsBase(7, 4, 12, 87));
  ...
  it('createComment POSTs the body to the comments endpoint', async () => {
    const promise = firstValueFrom(api.createComment(7, 4, 12, 87, { body: 'new comment' }));
    const req = httpMock.expectOne(commentsBase(7, 4, 12, 87));
  ...
  it('updateComment PATCHes the body', async () => {
    const promise = firstValueFrom(api.updateComment(7, 4, 12, 87, 311, { body: 'edited' }));
    const req = httpMock.expectOne(`${commentsBase(7, 4, 12, 87)}/311`);
  ...
  it('deleteComment DELETEs the comment endpoint', async () => {
    const promise = firstValueFrom(api.deleteComment(7, 4, 12, 87, 311));
    const req = httpMock.expectOne(`${commentsBase(7, 4, 12, 87)}/311`);
