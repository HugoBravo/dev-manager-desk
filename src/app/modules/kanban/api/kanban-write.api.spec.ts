...
const cardsBase = (projectId: number, taskId: number, boardId: number, columnId: number) =>
  `${FULL_PREFIX}/projects/${projectId}/tasks/${taskId}/kanban/boards/${boardId}/columns/${columnId}/cards`;
...
describe('KanbanWriteApi', () => {
  ...
  describe('createCard()', () => {
    it('POSTs to /columns/{c}/cards under the task-scoped URL chain', async () => {
      const promise = api.createCard(7, 9, 4, 12, { title: 'New card', body: '## Hello' }).toPromise();
      const req = httpMock.expectOne(cardsBase(7, 9, 4, 12));
    ...
    it('routes 422 field errors through the normalizer (W3 enforcement)', async () => {
      const promise = api.createCard(7, 9, 4, 12, { title: '' }).toPromise();
      httpMock.expectOne(cardsBase(7, 9, 4, 12)).flush(
        {
  ...
  describe('updateCard()', () => {
    it('PATCHes /cards/{card} under the task-scoped URL chain', async () => {
      const promise = api.updateCard(7, 9, 4, 12, 87, { title: 'Renamed' }).toPromise();
      const req = httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87`);
  ...
  describe('moveCard()', () => {
    it('POSTs /cards/{card}/move with to_column_id and returns server position', async () => {
      const promise = api.moveCard(7, 9, 4, 12, 87, { to_column_id: 15 }).toPromise();
      const req = httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87/move`);
      expect(req.request.method).toBe('POST');
    ...
    it('routes 422 position_exhausted through the normalizer with code', async () => {
      const promise = api.moveCard(7, 9, 4, 12, 87, { to_column_id: 15 }).toPromise();
      httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87/move`).flush(
        {
    ...
    it('does NOT mutate local state — caller must await the response', async () => {
      ...
      let resolved = false;
      const result = api.moveCard(7, 9, 4, 12, 87, { to_column_id: 15 });
  ...
  describe('archiveCard() / restoreCard()', () => {
    it('archiveCard POSTs /cards/{card}/archive under the task-scoped URL chain', async () => {
      const promise = api.archiveCard(7, 9, 4, 12, 87).toPromise();
      const req = httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87/archive`);
      expect(req.request.method).toBe('POST');
    ...
    it('restoreCard POSTs /cards/{card}/restore under the task-scoped URL chain', async () => {
      const promise = api.restoreCard(7, 9, 4, 12, 87).toPromise();
      const req = httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87/restore`);
      expect(req.request.method).toBe('POST');
  ...
  describe('deleteCard()', () => {
    it('DELETEs /cards/{card} and returns void on 204 (task-scoped URL chain)', async () => {
      const promise = api.deleteCard(7, 9, 4, 12, 87).toPromise();
      const req = httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87`);
    ...
    it('routes 404 through the normalizer (existence-leak prevention)', async () => {
      const promise = api.deleteCard(7, 9, 4, 12, 87).toPromise();
      httpMock
        .expectOne(`${cardsBase(7, 9, 4, 12)}/87`)
  ...
  describe('syncCardLabels()', () => {
    it('PUTs /cards/{card}/labels with label_ids and returns the updated card', async () => {
      const promise = api.syncCardLabels(7, 9, 4, 12, 87, [4, 7]).toPromise();
      const req = httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87/labels`);
    ...
    it('sends an empty array when labelIds is empty (clears the card)', async () => {
      const promise = api.syncCardLabels(7, 9, 4, 12, 87, []).toPromise();
      const req = httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87/labels`);
    ...
    it('routes 422 cross-user label id through the normalizer with fieldErrors', async () => {
      const promise = api.syncCardLabels(7, 9, 4, 12, 87, [4, 999]).toPromise();
      httpMock.expectOne(`${cardsBase(7, 9, 4, 12)}/87/labels`).flush(
        {
          message: 'The given data was invalid.',
  ...
  describe('column CRUD', () => {
    ...
    const columnsBase = (projectId: number, taskId: number, boardId: number) =>
      `${FULL_PREFIX}/projects/${projectId}/tasks/${taskId}/kanban/boards/${boardId}/columns`;

    it('createColumn POSTs /columns under the task-scoped URL chain', async () => {
      const promise = api.createColumn(7, 9, 4, { name: 'Backlog' }).toPromise();
      const req = httpMock.expectOne(columnsBase(7, 9, 4));
      expect(req.request.method).toBe('POST');
    ...
    it('updateColumn PATCHes /columns/{c} with the partial body (rename)', async () => {
      const promise = api.updateColumn(7, 9, 4, 21, { name: 'Renamed' }).toPromise();
      const req = httpMock.expectOne(`${columnsBase(7, 9, 4)}/21`);
      expect(req.request.method).toBe('PATCH');
    ...
    it('updateColumn PATCHes /columns/{c} with archived_at: null for unarchive', async () => {
      const promise = api.updateColumn(7, 9, 4, 21, { archived_at: null }).toPromise();
      const req = httpMock.expectOne(`${columnsBase(7, 9, 4)}/21`);
      expect(req.request.method).toBe('PATCH');
    ...
    it('updateColumn routes 409 column_has_contents through the normalizer', async () => {
      const promise = api
        .updateColumn(7, 9, 4, 21, { archived_at: '2026-07-07T00:00:00Z' })
        .toPromise();
      httpMock.expectOne(`${columnsBase(7, 9, 4)}/21`).flush(
        {
    ...
    it('deleteColumn DELETEs /columns/{c} and returns void on 204', async () => {
      const promise = api.deleteColumn(7, 9, 4, 21).toPromise();
      const req = httpMock.expectOne(`${columnsBase(7, 9, 4)}/21`);
      expect(req.request.method).toBe('DELETE');
      req.flush(null, { status: 204, statusText: 'No Content' });
      await expect(promise).resolves.toBeUndefined();
    ...
    it('reorderColumns POSTs /columns/reorder with { ordered_ids } and unwraps the result', async () => {
      const promise = api.reorderColumns(7, 9, 4, [21, 15, 12]).toPromise();
      const req = httpMock.expectOne(`${columnsBase(7, 9, 4)}/reorder`);
      expect(req.request.method).toBe('POST');
    ...
    it('reorderColumns spreads a readonly array into the request body', async () => {
      ...
      const promise = api.reorderColumns(7, 9, 4, ids).toPromise();
      const req = httpMock.expectOne(`${columnsBase(7, 9, 4)}/reorder`);
      expect(req.request.body).toEqual({ ordered_ids: [21, 15, 12] });
      req.flush({ reordered: 3 });
      await expect(promise).resolves.toEqual({ reordered: 3 });
    ...
    it('moveColumn POSTs /columns/{c}/move with { to_board_id } and returns the column', async () => {
      const promise = api.moveColumn(7, 9, 4, 21, 9).toPromise();
      const req = httpMock.expectOne(`${columnsBase(7, 9, 4)}/21/move`);
      expect(req.request.method).toBe('POST');
  ...
  describe('board CRUD', () => {
    ...
    const boardsBase = (projectId: number, taskId: number) =>
      `${FULL_PREFIX}/projects/${projectId}/tasks/${taskId}/kanban/boards`;

    describe('createBoard()', () => {
      it('POSTs /projects/{p}/tasks/{t}/kanban/boards with { name } and returns the new board', async () => {
        const promise = api.createBoard(7, 9, { name: 'Sprint 42' }).toPromise();
        const req = httpMock.expectOne(boardsBase(7, 9));
        expect(req.request.method).toBe('POST');
      ...
      it('routes 422 name_taken through the normalizer with fieldErrors', async () => {
        const promise = api.createBoard(7, 9, { name: 'taken' }).toPromise();
        httpMock.expectOne(boardsBase(7, 9)).flush(
          {
    ...
    describe('updateBoard()', () => {
      it('PATCHes /boards/{id} with the partial payload under the task-scoped URL chain', async () => {
        const promise = api.updateBoard(7, 9, 4, { name: 'Renamed' }).toPromise();
        const req = httpMock.expectOne(`${FULL_PREFIX}/projects/7/tasks/9/kanban/boards/4`);
        expect(req.request.method).toBe('PATCH');
        expect(req.request.body).toEqual({ name: 'Renamed' });
      ...
      it('routes 422 name_taken on rename through the normalizer', async () => {
        const promise = api.updateBoard(7, 9, 4, { name: 'taken' }).toPromise();
    ...
    describe('deleteBoard()', () => {
      it('DELETEs /boards/{id} and returns void on 204 (task-scoped URL chain)', async () => {
        const promise = api.deleteBoard(7, 9, 4).toPromise();
        const req = httpMock.expectOne(`${FULL_PREFIX}/projects/7/tasks/9/kanban/boards/4`);
        expect(req.request.method).toBe('DELETE');
        req.flush(null, { status: 204, statusText: 'No Content' });
        await expect(promise).resolves.toBeUndefined();
    ...
    describe('restoreBoard()', () => {
      it('POSTs /boards/{id}/restore and returns the restored board (task-scoped URL chain)', async () => {
        const promise = api.restoreBoard(7, 9, 4).toPromise();
        const req = httpMock.expectOne(`${FULL_PREFIX}/projects/7/tasks/9/kanban/boards/4/restore`);
        expect(req.request.method).toBe('POST');
      ...
      it('routes 422 not_trashed through the normalizer as validation', async () => {
        const promise = api.restoreBoard(7, 9, 4).toPromise();
        httpMock.expectOne(`${FULL_PREFIX}/projects/7/tasks/9/kanban/boards/4/restore`).flush(
          {
    ...
    describe('cloneBoard()', () => {
      it('POSTs /boards/{id}/clone with no body and returns the new board (task-scoped URL chain)', async () => {
        const promise = api.cloneBoard(7, 9, 4).toPromise();
        const req = httpMock.expectOne(`${FULL_PREFIX}/projects/7/tasks/9/kanban/boards/4/clone`);
        expect(req.request.method).toBe('POST');
      ...
      it('POSTs /boards/{id}/clone with { name } when provided (task-scoped URL chain)', async () => {
        const promise = api.cloneBoard(7, 9, 4, { name: 'Q3 Sprint' }).toPromise();
        const req = httpMock.expectOne(`${FULL_PREFIX}/projects/7/tasks/9/kanban/boards/4/clone`);
      ...
      it('routes 404 cross-owner / trashed source through the normalizer', async () => {
        const promise = api.cloneBoard(7, 9, 4).toPromise();
        httpMock
          .expectOne(`${FULL_PREFIX}/projects/7/tasks/9/kanban/boards/4/clone`)
          .flush({ message: 'gone' }, { status: 404, statusText: 'Not Found' });
        await expect(promise).rejects.toMatchObject({ kind: 'notFound' });
      });
    });
